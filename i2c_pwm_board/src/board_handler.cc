...
BoardHandler::BoardHandler(const std::string &node_name) {
  this->board_node = std::make_shared<BoardNode>(node_name);
}
...
void BoardHandler::set_handlers(int board_number) {
  ...
  if (board_number == 1) {
    ...
    this->config_srv = this->board_node->create_service<i2c_pwm_board_msgs::srv::ServosConfig>("front_config_servos",
    ...
    this->abs_sub = this->board_node->create_subscription<i2c_pwm_board_msgs::msg::ServoArray>("front_servos_absolute",
                                                                                       500,
    ...
    this->rel_sub = this->board_node->create_subscription<i2c_pwm_board_msgs::msg::ServoArray>("front_servos_proportional",
                                                                                       500,
    ...
    this->freq_srv = this->board_node->create_service<i2c_pwm_board_msgs::srv::IntValue>("front_set_pwm_frequency",
                                                                                 std::bind(&BoardHandler::set_pwm_frequency_handler,
                                                                                           this,
                                                                                           std::placeholders::_1,
                                                                                           std::placeholders::_2));
    this->mode_srv = this->board_node->create_service<i2c_pwm_board_msgs::srv::DriveMode>("front_config_drive_mode",
    ...
    this->config_srv = this->board_node->create_service<i2c_pwm_board_msgs::srv::ServosConfig>("back_config_servos",
                                                                                       std::bind(&BoardHandler::config_servos_handler,
                                                                                                 this,
                                                                                                 std::placeholders::_1,
                                                                                                 std::placeholders::_2));
    this->abs_sub = this->board_node->create_subscription<i2c_pwm_board_msgs::msg::ServoArray>("back_servos_absolute",
                                                                                       500,
                                                                                       std::bind(&BoardHandler::servos_absolute_handler,
                                                                                                 this,
                                                                                                 std::placeholders::_1));
    this->rel_sub = this->board_node->create_subscription<i2c_pwm_board_msgs::msg::ServoArray>("back_servos_proportional",
                                                                                       500,
                                                                                       std::bind(&BoardHandler::servos_proportional_handler,
                                                                                                 this,
                                                                                                 std::placeholders::_1));

    this->freq_srv = this->board_node->create_service<i2c_pwm_board_msgs::srv::IntValue>("back_set_pwm_frequency",
                                                                                 std::bind(&BoardHandler::set_pwm_frequency_handler,
                                                                                           this,
                                                                                           std::placeholders::_1,
                                                                                           std::placeholders::_2));

    this->mode_srv = this->board_node->create_service<i2c_pwm_board_msgs::srv::DriveMode>("back_config_drive_mode",
                                                                                  std::bind(&BoardHandler::config_drive_mode_handler,
                                                                                            this,
                                                                                            std::placeholders::_1,
                                                                                            std::placeholders::_2));
...
void BoardHandler::servos_drive_handler(const std::shared_ptr<geometry_msgs::msg::Twist> msg) {
  ...
  if (this->board_node->get_active_drive().mode == MODE_UNDEFINED) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "drive mode not set");
    return;
  }
  if ((this->board_node->get_active_drive().mode < MODE_UNDEFINED)
      || (this->board_node->get_active_drive().mode >= MODE_INVALID)) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),
  ...
  for (i = 0; i < (this->board_node->get_last_servo()); i++) {
    ...
    switch (this->board_node->get_active_drive().mode) {
      case MODE_MECANUM:
        if (this->board_node->servo_configs[i].mode_pos == POSITION_RIGHTREAR)
          this->board_node->set_pwm_interval_proportional(i + 1, speed[3]);
        if (this->board_node->servo_configs[i].mode_pos == POSITION_LEFTREAR)
          this->board_node->set_pwm_interval_proportional(i + 1, speed[2]);
        break; // This may cause an issue.   ^
        //                            |
      case MODE_DIFFERENTIAL:
        if (this->board_node->servo_configs[i].mode_pos == POSITION_RIGHTFRONT)
          this->board_node->set_pwm_interval_proportional(i + 1, speed[1]);
        break; // This may cause an issue.
      case MODE_ACKERMAN:
        if (this->board_node->servo_configs[i].mode_pos == POSITION_LEFTFRONT)
